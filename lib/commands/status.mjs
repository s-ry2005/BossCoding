...
function fileMatches(file, expected, executable = false) {
  try {
    ...
    // Git 在 Windows 的 autocrlf 会在 checkout 时规范换行；这不是技能被篡改。
    // 仍逐字比较其余内容，且 symlink、缺失和 Unix 可执行位照常视为不合格。
...
function frameworkAssets(abs) {
  const missingTemplates = [
    ...HOOKS.map(({ template }) => `templates/hooks/${template}`),
    ...SKILLS.map((skill) => `templates/skills/${skill}/SKILL.md`),
    ".github/workflows/bosscoding.yml",
  ].filter((rel) => {
    try {
      return !fs.lstatSync(path.join(abs, rel)).isFile();
    } catch {
      return true;
    }
  });
  return {
    hooksReady: missingTemplates.every((rel) => !rel.startsWith("templates/hooks/")),
    missingHooks: missingTemplates.filter((rel) => rel.startsWith("templates/hooks/")),
    skillsReady: missingTemplates.every((rel) => !rel.startsWith("templates/skills/")),
    missingSkills: missingTemplates.filter((rel) => rel.startsWith("templates/skills/")),
    ciReady: !missingTemplates.includes(".github/workflows/bosscoding.yml"),
  };
}

function isFrameworkRepository(abs, packageState) {
  if (packageState.pkg?.name !== packageIdentity().name) return false;
  try {
    return fs.lstatSync(path.join(abs, "templates")).isDirectory() &&
      fs.lstatSync(path.join(abs, "bin", "bosscoding.mjs")).isFile();
  } catch {
    return false;
  }
}
...
export function probe(root, options = {}) {
  ...
  const frameworkMode = isFrameworkRepository(abs, packageState);
  ...
  const assets = frameworkMode ? frameworkAssets(abs) : localAssets(abs, hooksDir);
...
export function runStatus(root = process.cwd(), options = {}) {
  ...
  if (s.frameworkMode) {
    console.log(paint.bold("BossCoding 框架维护模式："));
    console.log("  这里保存的是安装到产品项目的模板；不要求把 hooks 或技能副本安装到框架仓库自身。");
    console.log(`  ${s.hooksReady ? "✓" : "✗"} 三个 hook 模板${s.hooksReady ? "已就位" : `缺失：${s.missingHooks.join("、")}`}`);
    console.log(`  ${s.skillsReady ? "✓" : "✗"} 三个 AI 技能模板${s.skillsReady ? "已就位" : `缺失：${s.missingSkills.join("、")}`}`);
    console.log(`  ${s.ciReady ? "✓" : "✗"} GitHub 质检模板${s.ciReady ? "已就位" : "缺失：.github/workflows/bosscoding.yml"}`);
    console.log(`  ${s.testEntryConfigured ? "✓" : "○"} 框架测试入口${s.testEntryConfigured ? "已配置" : "未配置"}`);
    console.log(paint.bold("\n下一步："));
    console.log("  修改框架时走独立分支和 PR；发布前运行 npm run preflight。新产品项目执行 init 后，再用 status 检查已安装的 hooks 和技能。");
    return s.hooksReady && s.skillsReady && s.ciReady && s.testEntryConfigured ? 0 : 1;
  }
  ...
  if (!s.rulesReady) {
    ...
    console.log("  对 AI 说：「运行 npx -y @s-ry2005/bosscoding@latest update，恢复缺失或过期的本地门禁和 AI 技能，然后重新检查状态。」");
  } else if (!s.hasCommit) {
