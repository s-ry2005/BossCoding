...
默认使用当前原生系统环境。Windows 项目以本地检查和 GitHub Ubuntu CI 为准；不得因为类 Unix 差异自行切到 WSL、Ubuntu、容器或另一套工作区。只有老板明确说「本任务使用 WSL/Ubuntu」时，才启用本地 Unix 路径，并在任务开头报告实际环境；环境差异仍不能代替测试或 CI。

1. 开工前完整阅读项目根目录的 `AGENTS.md`，全程遵守导师模式和红线。拿不准真实状态，就用项目选定的包管理器执行 `bosscoding status`，不要读文档猜。
2. 从稳定分支（新项目默认叫 main）开任务分支。并行的第二个任务起，用项目选定的包管理器执行 `bosscoding task <任务名>`，准备独立工作区；平台支持创建任务或会话时，必须同时创建绑定该工作区的独立交互任务并交接上下文。只有平台确实不支持时，才用一句话请老板打开它；不要让老板切目录、搬文件或清理工作区。
